...
const DESKTOP_TOOLS: ReadonlySet<string> = new Set([
  ...
  "openFolder", "listFiles", "searchFiles", "searchPcWide", "editFile",
  ...
  "moveCursor", "mouseClick", "typeText", "pressKey", "sendHotkey", "scrollMouse", "scrollSmooth", "clickDragSelect", "mouseDragSelect",
  ...
  // UI Automation element tools (V4)
  "clickUIAElement", "findUIAElement", "listUIElements",
]);
          ...
          tools: [
            {
              functionDeclarations: [
                ...
                // --- V4: UI Automation (Accessibility tree) element tools ---
                {
                  name: "clickUIAElement",
                  description: "Find and click a UI element via the Windows Accessibility tree (UIA). More accurate than pixel/OCR clicking for standard Windows controls like menus, buttons, menu items, checkboxes. PREFER this for Windows app controls when a visible text label is known.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      text: { type: Type.STRING, description: "Visible label of the element, e.g. 'File', 'Save', 'OK'." },
                      control_type: { type: Type.STRING, description: "Optional control type filter: Button, MenuItem, CheckBox, Edit, TabItem, ListItem, etc." },
                      exact: { type: Type.BOOLEAN, description: "Require exact name match (default false = fuzzy/substring)." },
                      double: { type: Type.BOOLEAN, description: "Double-click (default false)." },
                      search_depth: { type: Type.INTEGER, description: "Tree search depth from foreground window (default 5)." }
                    },
                    required: ["text"]
                  }
                },
                {
                  name: "findUIAElement",
                  description: "Find a UI element by name/control type via the Accessibility tree WITHOUT clicking. Returns its screen coordinates and bounding box.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      text: { type: Type.STRING, description: "Element label to locate." },
                      control_type: { type: Type.STRING, description: "Optional control type filter (Button, MenuItem, ...)." },
                      exact: { type: Type.BOOLEAN, description: "Require exact match (default false)." },
                      search_depth: { type: Type.INTEGER, description: "Search depth (default 5)." }
                    },
                    required: ["text"]
                  }
                },
                {
                  name: "listUIElements",
                  description: "List the interactive UI elements of the foreground window from the Accessibility tree. Useful for understanding what controls are available before clicking.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      control_types: { type: Type.ARRAY, items: { type: Type.STRING }, description: "Optional filter: only list these control types (e.g. ['Button','MenuItem'])." },
                      name_filter: { type: Type.STRING, description: "Optional substring filter on element name." },
                      max_depth: { type: Type.INTEGER, description: "Tree walk depth (default 3)." },
                      limit: { type: Type.INTEGER, description: "Max elements to return (default 60)." }
                    }
                  }
                },
