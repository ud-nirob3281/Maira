...
async def _clean_close_async() -> None:
    """Cleanly close Playwright context and process so user profile folders can unlock."""
...
def clean_close() -> None:
    """Synchronous entry point to cleanly close the browser."""
    try:
        _run(_clean_close_async())
    except Exception:
        STATE.reset_playwright()
...
async def _ensure_browser_async() -> Any:
    if STATE.page is not None:
        try:
            is_closed = False
            try:
                is_closed = STATE.page.is_closed()
            except Exception:
                try:
                    is_closed = STATE.page.is_closed
                except Exception:
    ...
    try:
        ...
        await _clean_close_async()
        raise ToolError(f"Failed to retrieve or create page: {e}")
...
async def _retry_browser_op(op_name: str, coro_func, retries: int = 1):
    """Run a browser operation creator, resetting the browser context on failure and retrying."""
    last_err = None
...
async def browser_tab_action(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if action == "new":
        ...
        target_title = args.get("title") or args.get("text")
        
        # 1. Switch by index if provided
        if target_index is not None:
            try:
                ...
                if 0 <= idx < len(pages):
                    ...
                    return {"result": f"Switched to tab index {idx}. Currently on: {STATE.page.url}", "url": STATE.page.url}
                else:
                    return {"result": f"Index {idx} out of range (0-{len(pages)-1})."}
            except (ValueError, TypeError):
                pass
                
        # 2. Switch by url or title matching
        if target_url or target_title:
            for i, p in enumerate(pages):
                try:
                    p_url = p.url or ""
                    p_title = await p.title() or ""
                    if (target_url and target_url.lower() in p_url.lower()) or \
                       (target_title and target_title.lower() in p_title.lower()):
                        STATE.page = p
                        await p.bring_to_front()
                        return {"result": f"Switched to tab matching URL/Title. Currently on: {STATE.page.url}", "url": STATE.page.url}
                except Exception:
                    continue
                    
        # 3. Default to cycling to next tab
        current_idx = 0
        for i, p in enumerate(pages):
            if p == STATE.page:
                current_idx = i
        ...
        STATE.page = pages[next_idx]
        await STATE.page.bring_to_front()
        return {"result": f"Cycled to next tab (index {next_idx}). Currently on: {STATE.page.url}", "url": STATE.page.url}
