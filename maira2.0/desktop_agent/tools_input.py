...
def _pixel_to_abs(x: int, y: int) -> Tuple[int, int]:
    ...
    # Map to 0-65535
    div_w = (vw - 1) if vw > 1 else 1
    div_h = (vh - 1) if vh > 1 else 1
    ax = int((px - vx) * 65535 / div_w)
    ay = int((py - vy) * 65535 / div_h)
    return ax, ay


def _win32_move(x: int, y: int) -> None:
    """Move cursor to absolute screen coordinates (x, y) via physical SetCursorPos and SendInput."""
    _user32.SetCursorPos(x, y)
    ax, ay = _pixel_to_abs(x, y)
    _send_mouse_input(ax, ay, MOUSEEVENTF_MOVE | MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_VIRTUALDESK)


def _win32_click(button: str, x: Optional[int] = None, y: Optional[int] = None) -> None:
    """Send a complete mouse button down + up via SendInput at optional (x, y)."""
    down, up = _BUTTON_FLAGS.get(button, (MOUSEEVENTF_LEFTDOWN, MOUSEEVENTF_LEFTUP))
    if x is not None and y is not None:
        _user32.SetCursorPos(x, y)
        ax, ay = _pixel_to_abs(x, y)
        _send_mouse_input(ax, ay, down | MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_VIRTUALDESK)
        time.sleep(0.05)
        _send_mouse_input(ax, ay, up | MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_VIRTUALDESK)
    else:
        _send_mouse_input(0, 0, down)
        time.sleep(0.05)  # tiny gap so the OS registers a distinct press+release
        _send_mouse_input(0, 0, up)


def _win32_double_click(button: str, x: Optional[int] = None, y: Optional[int] = None) -> None:
    """Send two rapid clicks via SendInput at optional (x, y)."""
    down, up = _BUTTON_FLAGS.get(button, (MOUSEEVENTF_LEFTDOWN, MOUSEEVENTF_LEFTUP))
    if x is not None and y is not None:
        _user32.SetCursorPos(x, y)
        ax, ay = _pixel_to_abs(x, y)
...
def _win32_mouse_down(button: str, x: Optional[int] = None, y: Optional[int] = None) -> None:
...
def _win32_mouse_up(button: str, x: Optional[int] = None, y: Optional[int] = None) -> None:
...
def _win32_hscroll(amount: int, direction: str) -> None:
    ...
    if direction == "left":
        delta = -delta
    _send_mouse_input(0, 0, MOUSEEVENTF_HWHEEL, delta << 16)
...
def _click_with_retry(x: int, y: int, button: str, clicks: int, interval_s: float) -> bool:
    """Move to (x,y) then click via SendInput, retrying once if position shifted."""
    for attempt in range(2):
        _smooth_move(x, y)
        time.sleep(0.08)
        if clicks > 1:
            _win32_double_click(button, x, y)
        else:
            _win32_click(button, x, y)
        # Verify cursor stayed on target
        time.sleep(0.05)
        ax, ay = _win32_get_pos()
        if abs(ax - x) <= 10 and abs(ay - y) <= 10:
            return True
        time.sleep(0.15)
...
def move_cursor(args: Dict[str, Any]) -> Dict[str, Any]:
    """Move the mouse pointer to absolute screen coordinates (physical pixels) or target text."""
    target = args.get("text") or args.get("target") or args.get("label")
    if target:
        from .tools_visual import _find_element_visual
        el = _find_element_visual(str(target))
        if el:
            x, y = el["x"], el["y"]
            _smooth_move(x, y)
            return {"result": f"Moved cursor to '{target}' at ({x}, {y}) via {el['method']}.", "x": x, "y": y, "method": el["method"]}
        else:
            raise ToolError(f"Could not visually locate '{target}' on the screen.")
...
def mouse_click(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    target = args.get("text") or args.get("target") or args.get("label")
    if target:
        from .tools_visual import click_on_text
        return click_on_text(args)
...
def mouse_drag(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    from_text = args.get("from_text") or args.get("text1") or args.get("start_text")
    to_text = args.get("to_text") or args.get("text2") or args.get("end_text")

    if from_text:
        from .tools_visual import _find_element_visual
        from_el = _find_element_visual(str(from_text))
        if from_el:
            args["x1"], args["y1"] = from_el["x"], from_el["y"]
        else:
            raise ToolError(f"Could not visually find start target '{from_text}' for dragging.")

    if to_text:
        from .tools_visual import _find_element_visual
        to_el = _find_element_visual(str(to_text))
        if to_el:
            args["x2"], args["y2"] = to_el["x"], to_el["y"]
        else:
            raise ToolError(f"Could not visually find end target '{to_text}' for dragging.")

    x1 = args.get("x1", args.get("from_x", args.get("fromX", args.get("startX", args.get("start_x")))))
    y1 = args.get("y1", args.get("from_y", args.get("fromY", args.get("startY", args.get("start_y")))))
    x2 = args.get("x2", args.get("to_x", args.get("toX", args.get("endX", args.get("end_x")))))
    y2 = args.get("y2", args.get("to_y", args.get("toY", args.get("endY", args.get("end_y")))))
    button = str(args.get("button", "left")).lower()
    ...
    _win32_mouse_down(button, x1, y1)
...
def scroll_mouse(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    If x and y coordinates are provided, moves the cursor there first.
    """
...
def scroll_until_visible(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for i in range(max_scrolls):
        ...
        # Invalidate capture cache so the next OCR pass gets a fresh frame.
        try:
...
@register("clickDragSelect")
@register("mouseDragSelect")
def click_drag_select(args: Dict[str, Any]) -> Dict[str, Any]:
    """Perform a click and drag gesture to select text or items from (x1, y1) to (x2, y2)."""
    return mouse_drag(args)
