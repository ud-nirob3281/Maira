...
import { analyzeAndSplitUserRequest, ResultCollector } from "./server_scheduler";
import {
  DATA_DIR,
  dataFile,
  getGeminiApiKey,
...
export type SabitTaskStatus = "idle" | "acquiring" | "running" | "waiting_for_user" | "recovering" | "completed" | "failed" | "cancelled";
  ...
  if (globalWss && globalWss.clients) {
    for (const client of globalWss.clients) {
      if (client.readyState === 1 /* OPEN */) {
        ...
        try { client.send(legacyPayload); } catch (e) {}
      }
    }
  }
...
export function cancelSabitTask(reason: string = "Task explicitly cancelled by user."): boolean {
...
let activeSabitLiveSession: any = null;
let activeSabitToolCall: any = null;
let activeSabitClientWs: any = null;
let activeMairaLiveSession: any = null;
...
/**
 * Maira's FULL persona instructions — byte-for-byte restoration of Maira1's
 ...
 */
function getMairaFullInstructions(assistantName: string, activeGoal?: string): string {
...
async function startServer() {
  ...
  app.post("/api/sabit-manual-state", express.json(), (req, res) => {
    ...
    if (disconnected) {
      ...
      isCurrentlyDelegated = false;
      if (activeSabitLiveSession) {
        try { activeSabitLiveSession.close(); } catch (e) {}
...
- System Commands Access (shutdown, restart, sleep, power actions): ENABLED.`;
          ...
          tools: [
            {
              functionDeclarations: SHARED_TOOL_DECLARATIONS
                ? SHARED_TOOL_DECLARATIONS.filter(t => t.name !== "delegateToSabit")
                : []
            }
          ]
        },
            ...
            if (message.toolCall?.functionCalls) {
              ...
              for (const fc of message.toolCall.functionCalls) {
                ...
                if (fc.name === "sabitWaitingForUser") {
                  const msg = (fc.args as any)?.message || "User action required on screen.";
      ...
      const liveWatchdogInterval = setInterval(() => {
        ...
        if (isTaskActive && inFlightToolCallsCount === 0 && (now - lastActivityTimestamp > 8000)) {
...
` }]
  ...
  wss.on("connection", async (clientWs, request) => {
    ...
    try {
      ...
      const memories = await loadMemories();
      const rules = await loadLearnedRules();
      // Maira uses her FULL anime-heroine persona instructions (restored from
      // Maira1) — identical voice settings, no-repetition policy, cute expression
      // catalog, browser Golden Rule, WhatsApp protocol, screenshot viewport rules,
      // and anti-hallucination verification loop. This preserves her unique
      // personality. Sabit uses the generic getSharedAutomationBaseInstructions().
        ...
        callbacks: {
          onmessage: (message: LiveServerMessage) => {
            ...
            if (message.serverContent?.turnComplete) {
              ...
              // Maira Continuous Execution Driver for parallel / self-executed multi-step tasks
            ...
            if (userTextOutput) {
              ...
              // Run Task Scheduler analysis for compound spoken voice requests
              ...
              for (const fc of message.toolCall.functionCalls) {
                ...
                if (fc.name === "delegateToSabit") {
                  (async () => {
                    ...
                    const isSabitBusy = sabitRuntimeState.taskState === "acquiring" || sabitRuntimeState.taskState === "running" || sabitRuntimeState.taskState === "recovering" || sabitRuntimeState.taskState === "waiting_for_user";
                    if (isSabitBusy) {
...
` }]
                  ...
                  // Force-delegation guard: If Sabit is connected and available and idle, block browser automation tools from direct execution by Maira if single task
                  const BROWSER_AUTOMATION_TOOLS = new Set([
                    "openWebsite", "searchWeb", "searchYouTube", "searchGoogle", "searchGitHub",
                    "desktopBrowserOpen", "desktopBrowserSnapshot", "desktopBrowserClick", "desktopBrowserType",
                    "desktopBrowserSearch", "desktopBrowserScroll", "desktopBrowserGetText", "desktopBrowserScreenshot",
                    "desktopBrowserMediaControl", "desktopBrowserPressKey", "desktopBrowserListTabs", "desktopBrowserSwitchTab"
                  ]);
        ...
        try {
          ...
          if (msg.type === "ping") {
           ...
           if (msg.audio) {
            ...
            // Chat text input from user → forward to Gemini Live session via Scheduler
            try {
