...
def _resolve_locator(w: "_BrowserWorker", ref=None, selector=None, text=None, role=None, name=None):
    ...
    if ref:
        ...
        if nm:
            def s_role_name(r=r, nm=nm, nth=nth):
                loc = page.get_by_role(r, name=nm, exact=False)
                return loc.nth(nth) if nth is not None else loc.first
            strategies.append(("role + name", s_role_name))
            
            # Strategy B: text search fallback (since role classification might vary in DOM)
            def s_text(nm=nm, nth=nth):
                loc = page.get_by_text(nm, exact=False)
                return loc.nth(nth) if nth is not None else loc.first
            strategies.append(("text search", s_text))
