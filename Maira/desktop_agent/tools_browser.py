...
def _resolve_locator(w: "_BrowserWorker", ref=None, selector=None, text=None, role=None, name=None):
    ...
    if ref:
        if ref not in w.element_map:
            raise ToolError(f"Element reference '{ref}' is invalid or expired. Please call browserSnapshot to refresh accessible elements.")
        ...
        strategies = []
        # Strategy A: role + name + nth (exact match first)
        if nm:
            def s_role_name_exact(r=r, nm=nm, nth=nth):
                loc = page.get_by_role(r, name=nm, exact=True)
                return loc.nth(nth) if nth is not None else loc.first
            ...
            # Strategy B: text search fallback (exact match first)
        ...
        raise ToolError(
            f"Element '{ref}' (role: '{r}', name: '{nm}') was found in snapshot but is no longer on the page "
            f"or could not be reached via any lookup strategy (last error: {last_err}). "
            f"Please refresh accessible elements with browserSnapshot."
        )
...
def _browser_click(w: "_BrowserWorker", args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if text:
        attempts.append(lambda: w.page.get_by_text(str(text), exact=True).first)
        attempts.append(lambda: w.page.get_by_role("button", name=str(text), exact=True).first)
        attempts.append(lambda: w.page.get_by_role("link", name=str(text), exact=True).first)
