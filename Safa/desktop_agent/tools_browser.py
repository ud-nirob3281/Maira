...
class _BrowserWorker:
    ...
    _STOP = object()  # poison pill: tells a stale dispatch loop to exit
    ...
    def __init__(self) -> None:
        ...
        from .tools_snapshot_manager import SnapshotCacheManager
        self.snapshot_cache: SnapshotCacheManager = SnapshotCacheManager()
    ...
    def restart(self) -> None:
        """Hard-restart the worker thread with a FRESH Playwright instance.

        Used when Playwright's greenlet thread-affinity is corrupted — the
        classic symptom is `Cannot switch to a different thread` on every
        call, which happens after the previous worker thread died while a
        Playwright call was in flight. The old sync_playwright object is bound
        to a dead greenlet and can never be used again; only a brand-new
        thread + brand-new sync_playwright() recovers.
        """
        print("[BrowserWorker] Hard-restarting worker thread (fresh Playwright instance)…")
        # Ask any stale dispatch loop to exit (it may be blocked on queue.get).
        try:
            self._task_q.put((_BrowserWorker._STOP, None, None))
        except Exception:
            pass
        # Drop all references to the old (dead-greenlet-bound) objects. Closing
        # them from this thread would itself raise the same greenlet error.
        self.pw = None
        self.browser = None
        self.context = None
        self.page = None
        self.element_map = {}
        self._thread = None  # start() will spawn a fresh thread -> fresh _run()
        self.start()
...
SABIT_WORKER = _BrowserWorker()

def _get_snapshot_cache(w: Optional["_BrowserWorker"] = None):
    if w is not None and hasattr(w, "snapshot_cache") and w.snapshot_cache is not None:
        return w.snapshot_cache
...
def _profile_dir(w: Optional["_BrowserWorker"] = None) -> str:
    suffix = "sabit" if w is SABIT_WORKER else ""
    folder_name = "browser-profile" if not suffix else f"browser-profile-{suffix}"
    return os.path.join(os.environ.get("MYRAA_DATA_DIR", os.getcwd()), folder_name)
...
def _kill_profile_zombie_chromium(profile_dir: str) -> int:
    """Kill Chromium processes still holding our persistent profile dir.

    After a crash (or when a previous agent process died), zombie chrome.exe
    processes keep the profile's singleton lock, so the next
    launch_persistent_context fails with 'profile is already in use by another
    instance of Chromium'. Killing the holders frees the lock. Only processes
    whose command line references THIS profile dir are killed — the user's own
    Chrome/browser is never touched.
    """
    killed = 0
    ...
    for proc in psutil.process_iter(["pid", "name", "cmdline"]):
        try:
            ...
            if "chrom" not in name:  # chrome.exe / chromium.exe
                continue
            cmdline = " ".join(proc.info.get("cmdline") or [])
            if target in os.path.normcase(cmdline):
                proc.kill()
                killed += 1
    ...
    if killed:
        import time as _t
        _t.sleep(1.5)  # let the OS release the profile lock files
        print(f"[BrowserWorker] Killed {killed} zombie Chromium process(es) holding the profile.")
    return killed
...
def _ensure_browser(w: "_BrowserWorker", _args=None) -> Dict[str, Any]:
    ...
    if w.context is None or not context_healthy or w.page is None or not page_healthy:
        ...
        os.makedirs(_profile_dir(w), exist_ok=True)
        launch_args = [
            "--start-maximized",
            "--no-sandbox",
        ...
        try:
            ...
            msg = str(e)
            w.context = None
            # Profile lock held by zombie Chromium from a previous crashed
            # session — kill the holders and retry the launch once.
            if "already in use" in msg or "Opening in existing browser session" in msg:
                try:
                    _kill_profile_zombie_chromium(_profile_dir(w))
                    w.context = w.pw.chromium.launch_persistent_context(
                        _profile_dir(w),
                        headless=False,
                        args=launch_args,
                    ...
                    w.context = None
                    raise ToolError(f"Could not launch Chromium: {retry_err}") from retry_err
...
def _browser_snapshot(w: "_BrowserWorker", args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # The screen hash is one of several signals detect_screen_change() uses to
    # decide whether a cached snapshot is still valid. On WhatsApp (and other
    # SPAs) the visible UI can change while the DOM signature stays stable
    # (e.g. a chat opens, a new message arrives), so without the screen hash the
    # cache may serve stale refs and clicks/types land on the wrong element.
    # Mirrors Maira1's _browser_snapshot exactly.
    current_img = None
    if os.name == "nt":
        try:
    ...
    if not force and cache.check_and_update_state(w, current_img):
        if cache.cached_aria_snapshot is not None:
            # Restore the element map from cache!
            w.element_map = dict(cache.cached_aria_snapshot.get("element_map", {}))
            log_obj.info("Reusing cached ARIA snapshot.")
            return cache.cached_aria_snapshot["response"]
    ...
    try:
        ...
        # Settle delay: give the browser a moment to recompute the accessibility
        # tree after we mutate aria-label/title/placeholder to embed [ref=eN].
        # Without this, aria_snapshot() can return labels that still reflect the
        # OLD values, so the parsed element_map ends up sparse and subsequent
        # click/type-by-ref calls fail. This is a known cause of WhatsApp
        # automation breakage on heavy SPAs. Matches Maira1.
        time.sleep(0.1)
    ...
    # Stonic/OpenClaw-style nth disambiguation: elements sharing the same
    # role+name (e.g. three "Play" links) each get a distinct 0-based index,
    # recorded in the map and shown to the AI as [nth=N]. Click resolution
    # then targets the EXACT duplicate instead of "first visible".
    ...
    for line in aria_text.split("\n"):
        ...
        nth_idx = role_name_counts.get(key, 0)
        role_name_counts[key] = nth_idx + 1
...
def _resolve_locator(w: "_BrowserWorker", ref=None, selector=None, text=None, role=None, name=None):
    ...
    if ref:
        ...
        try:
            ...
            # Ref lost (dynamic re-render etc). FIRST try the Stonic-style
            # semantic re-resolution: role + name + nth from the snapshot map —
            # this targets the EXACT duplicate element without a re-snapshot.
            old_info = w.element_map.get(ref)
            if old_info and old_info.get("role"):
                try:
                    nth_val = old_info.get("nth")
                    if old_info.get("name"):
                        sem = page.get_by_role(
                            old_info["role"], name=old_info["name"], exact=True
                        )
                    else:
                        sem = page.get_by_role(old_info["role"])
                    if nth_val is not None:
                        sem = sem.nth(int(nth_val))
                    sem.first.wait_for(state="attached", timeout=1200)
                    if sem.count() > 0 or nth_val is not None:
                        return sem
                except Exception:
                    pass  # fall through to self-healing below
            ...
            target_nth = old_info.get("nth") if old_info else None
            ...
            # See if we can locate a fresh ref matching old role/name — prefer
            # the one with the SAME nth index (exact duplicate match).
            new_ref = None
            if target_role and target_name and target_name.strip():
                for r, info in w.element_map.items():
                    if (
                        info["role"] == target_role
                        and (info.get("name") or "").strip() == target_name.strip()
                        and (target_nth is None or info.get("nth") == target_nth)
                    ):
                        new_ref = r
                        break
                if not new_ref:
                    for r, info in w.element_map.items():
                        if info["role"] == target_role and info.get("name") and target_name.strip().lower() in (info.get("name") or "").lower():
                            new_ref = r
                            break
            ...
            elif old_info and target_name and target_name.strip():
                # Last-ditch effort: standard strategy lookup (nth-aware)
                try:
                    return _resolve_locator_with_strategies(w, old_info["role"], old_info.get("name"), target_nth)
                except Exception:
                    pass
            raise ToolError(f"Element reference '{ref}' is invalid or expired after a DOM update. Please call desktopBrowserSnapshot() to inspect current elements.")

    # 2. CSS selector
    if selector:
        import re as _re
        # Normalize model-provided ref-as-selector syntax: '[ref=e4]', '[ref="e4"]'
        # (elements are stamped with data-myraa-ref, not a plain ref attribute)
        _m = _re.fullmatch(r"\[ref\s*=\s*[\"']?([A-Za-z0-9_\-]+)[\"']?\]", selector.strip())
        if _m:
            selector = f"[data-myraa-ref='{_m.group(1)}']"
        ...
        try:
            ...
            try:
                if not loc.is_visible():
                    loc.scroll_into_view_if_needed(timeout=2000)
                    # Settle delay after scrolling: the page needs a moment to
                    # finish re-layout before we interact. Restored from Maira1
                    # (was 0.05 here, which raced the click on heavy SPAs).
                    time.sleep(0.3)
            ...
            # Settle delay after the click: WhatsApp (and similar SPAs) load the
            # next panel asynchronously (e.g. clicking a contact opens the chat
            # + footer message box). Restored from Maira1.
            time.sleep(0.6)
...
def _browser_type(w: "_BrowserWorker", args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # IMPORTANT: Keep this logic structurally identical to Maira1's
    # desktop_agent/tools_browser.py. Maira1's version is the proven, working
    # implementation. Both Maira and Sabit share this single Python tool, so
    # keeping it identical to Maira1 fixes both at once. Do NOT add custom
    # ref-override logic here — earlier attempts at that redirected the contact
    # name (typed into the search box with a ref) into the message footer,
    # which broke contact search entirely.
    if is_whatsapp and not ref and not selector:
        ...
        for sel in wa_message_selectors:
            try:
                ...
                # Defensive guard: after focusing, make sure the active element is
                # inside the footer (the message box) and NOT the search box.
                ...
                # Final verification: confirm the text really went into the
                # message box (footer), not the search box.
    ...
    def get_session_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "title": None,
                "tabs_count": 0,
                "profile_dir": _profile_dir(self.worker),
                "error": str(e)
            }
...
SABIT_SESSION_MANAGER = BrowserSessionManager(SABIT_WORKER)
...
def _with_recovery(tool_name: str, fn, args: Dict[str, Any]) -> Any:
    ...
    caller = args.get("_caller", "maira")
    w = SABIT_WORKER if caller == "sabit" else WORKER
    cache = _get_snapshot_cache(w)
    ...
    try:
        ...
        # Playwright greenlet thread-affinity corruption ("Cannot switch to a
        # different thread"): every subsequent call on this Playwright object
        # fails forever. Hard-restart the worker (fresh thread + fresh
        # sync_playwright) and retry once.
        if "cannot switch to a different thread" in msg or "greenlet" in msg:
            try:
                w.restart()
                res = w.call(fn, args)
                if not is_read_only:
                ...
                raise ToolError(f"Browser worker restarted but retry failed: {retry_err}")
...
def browser_session_status(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    caller = args.get("_caller", "maira")
    mgr = SABIT_SESSION_MANAGER if caller == "sabit" else SESSION_MANAGER
...
def browser_session_restore(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    mgr = SABIT_SESSION_MANAGER if caller == "sabit" else SESSION_MANAGER
    url = args.get("url")
    return mgr.restore_session(url)
