...
6. VISUAL HUB: You can generate visuals for the user — 'generate_diagram' (Mermaid.js v11, great for concepts/processes), 'render_math' (LaTeX steps for any math), 'render_chart' (line/bar/area/pie for data), 'generate_image' (AI pictures), 'generate_flashcards' (study revision). Prefer a visual whenever it explains better than text; tool descriptions have the exact formats.
7. EMOTION: Call 'express_emotion(emotion, intensity)' silently at the start of emotionally-colored turns so your on-screen video matches your voice; let the emotion genuinely shape your spoken delivery (soft/caring for pain, bright for joy, firm/controlled for serious matters).
8. CONCISE & POLITE RESPONSE: State your final answer clearly when the task is finished.`;
    ...
    while (stepCount < this.maxReActSteps) {
      ...
      try {
        const response = await ai.models.generateContent({
          ...
          config: {
            systemInstruction,
            // Visual Hub tools + emotion tool ride along so text-mode chat can
            // generate visuals and set the character emotion too — same
            // handlers as the Live session.
            tools: [{ functionDeclarations: [...(SHARED_TOOL_DECLARATIONS as any), ...(VISUAL_TOOL_DECLARATIONS as any), ...(EMOTION_TOOL_DECLARATIONS as any)] }]
          }
        });
            ...
            try {
              ...
              session.sendClientContent({
                turns: {
                  ...
                  parts: [
                    {
                      text: `SYSTEM DIRECTIVE (AGENT CORE RESULT): State the following result clearly to the user in voice, with natural emotional delivery matching the content (soft and caring for sensitive topics, warm and bright for good news, calm and steady for instructions): "${finalAnswer}"`
                    }
                  ]
                },
                turnComplete: true
        ...
        for (const fc of functionCalls) {
          ...
          if (fc.name === "memory" || fc.name === "memory_manage") {
            ...
            // Visual Hub tools — generation runs server-side in visual_tools.ts.
            // Push state changes over the client WS when available so the
            // panel live-updates even in text mode.
            try {
              const broadcast = (visual: any) => {
                try {
                  if (clientWs && clientWs.readyState === 1) {
                    clientWs.send(JSON.stringify({ type: "visual_hub_update", visual }));
                  }
                } catch {}
              };
              const { output } = await executeVisualTool(fc.name, fc.args, { onVisualUpdate: broadcast });
              toolResultOutput = output;
            } catch (err: any) {
              console.error("[AGENT CORE] Visual tool failure:", err);
              toolResultOutput = { error: "The visual could not be created. Politely tell the user and offer to try again." };
            }
          } else if (EMOTION_TOOL_NAMES.has(fc.name)) {
            // express_emotion — syncs the character's emotion (voice delivery
            // guidance comes from the system prompt; video via WS push).
            // Responds instantly and never fails the ReAct loop.
            try {
              const emotion = normalizeEmotion((fc.args as any)?.emotion);
              const intensity = normalizeIntensity((fc.args as any)?.intensity);
              if (clientWs && clientWs.readyState === 1) {
                clientWs.send(JSON.stringify({ type: "emotion", emotion, intensity }));
              }
              toolResultOutput = { result: "ok" };
            } catch {
              toolResultOutput = { result: "ok" };
            }
