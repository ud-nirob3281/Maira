...
/**
 * Get conversation messages across ALL sessions whose timestamp falls within
 * [fromISO, toISO] (inclusive). Powers "গতকাল কী কথা হয়েছিল?" style recall —
 * FTS keyword search cannot answer these because the message text rarely
 * contains the temporal phrase the user asked with. Only conversational
 * messages are returned (thinking/tool noise excluded).
 */
export function getSessionMessagesByDateRange(
  fromISO: string,
  toISO: string,
  limit: number = 40
): ChatMessageRecord[] {
  ...
  const rows = db.prepare(
    `SELECT m.*, s.title AS session_title
     ...
     LIMIT ?`
  ).all(fromISO, toISO, limit) as any[];
  return rows.map(rowToMessage);
}

/**
 * Distinct local dates (UTC+6) that have stored conversation messages, with
 * per-day counts. Powers "লাস্ট কোন দিনের ডাটা সেভ আছে?" answers and the
 * empty-date-range fallback ("that day has nothing stored; these days do").
 */
export function getAvailableConversationDates(limit: number = 30): Array<{ day: string; count: number }> {
  const db = getDb();
  const rows = db.prepare(
    `SELECT date(datetime(m.timestamp, '+6 hours')) AS day, COUNT(*) AS cnt
     FROM messages m
     WHERE m.role IN ('user', 'model') AND m.content != ''
     GROUP BY day
     ORDER BY day DESC
     LIMIT ?`
  ).all(limit) as any[];
  return rows.map(r => ({ day: String(r.day), count: Number(r.cnt) }));
}
