...
interface Secrets {
  geminiApiKey?: string;
  sabitApiKey?: string;
  /** User-added backup keys — used ONLY by the Visual section as a failover pool. */
  backupApiKeys?: BackupApiKey[];
}
...
// Stored alongside the main keys in the same secrets.json — same persistence,
// same owner-only permissions, same survival across restarts.
// ---------------------------------------------------------------------------

/** All saved backup keys, in insertion order. */
export function getBackupApiKeys(): BackupApiKey[] {
  return (readSecrets().backupApiKeys || []).filter(
    b => typeof b?.key === "string" && b.key.trim(),
  );
}

/** Overwrite the whole backup key list (CRUD lives at the route layer). */
export function setBackupApiKeys(keys: BackupApiKey[]): void {
  const dataSecrets = readSecrets();
  dataSecrets.backupApiKeys = keys;
  fs.writeFileSync(SECRETS_FILE, JSON.stringify(dataSecrets, null, 2), "utf-8");
  try {
