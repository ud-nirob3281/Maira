...
class SafaMemoryManager {
  ...
  assembleContext(params: {
    ...
    // Cross-session: if the current session is empty, only note that a previous
    // session EXISTS — never inject its transcript. Injecting the previous
    // session's full transcript made Safa resume the old session's last task in
    // a brand-new conversation. The model retrieves old content on demand via
    ...
    if (!hasRestore) {
      try {
        ...
        if (mostRecent && mostRecent.id !== sid) {
          parts.push(
            `[PREVIOUS SESSION NOTE — a previous session "${mostRecent.title || "Conversation"}" (last updated ${mostRecent.updated_at || "earlier"}) exists. Its contents are NOT loaded. Do NOT resume or mention its tasks unless the user asks about past conversations — then use the session_search tool.]`
          );
        }
      } catch (err: any) {
        console.error("[SafaMemoryManager] Error fetching cross-session note:", err.message);
      }
    }
...
export function formatSystemInstructionsWithContext(base: string, ...rest: any[]): string {
  ...
  // the model has continuity after a reconnect. This is more reliable than
  // replaying turns via sendClientContent on Gemini Live, which interrupts
  // itself turn-by-turn.
  //
  // SECURITY FIX: only the LAST 16 turns are injected, labeled as background —
  // NOT authoritative. Injecting 150 turns labeled "AUTHORITATIVE — continue
  // from here exactly" made the model resume the oldest unfinished task from
  // history instead of answering the user's CURRENT request. Older history is
  // NOT injected; the model retrieves it via the session_search tool when the
  // user actually asks about past conversations.
  const dialogueHistory = rest[2] as Array<{ role: string; text: string; timestamp?: string }> | undefined;
  if (Array.isArray(dialogueHistory) && dialogueHistory.length > 0) {
    const transcript = dialogueHistory
      ...
      .map((t) => {
        ...
        // Local (UTC+6) date+time prefix so the model can tell TODAY's turns
        // apart from earlier days — without this it relabeled current-session
        // topics as "yesterday's conversation".
        const ts = t.timestamp ? new Date(new Date(t.timestamp).getTime() + 6 * 3600e3).toISOString() : '';
        const stamp = ts ? `[${ts.slice(5, 10)} ${ts.slice(11, 16)}] ` : '';
        return `${stamp}${speaker}: ${(t.text || '').slice(0, 2000)}`;
      })
      .join('\n');
    result += `\n\n[RECENT CONVERSATION CONTEXT (background only — for continuity; each line is prefixed with its local date/time)]
${transcript}

CONTEXT RULES (CRITICAL):
1. The user's LATEST message is ALWAYS the current task. Answer THAT first.
2. NEVER resume, continue, or bring up older tasks/topics from this history unless the user explicitly asks about them.
3. If a task in this history was left unfinished AND the user's current message directly relates to it, you may ask AT MOST ONCE whether to continue it. If the user declines or changes topic, never mention it again.
4. When the user asks about a PAST time (গতকাল/yesterday/N days ago/a specific date), a separate "[ACTUAL STORED CONVERSATIONS FROM ...]" block will contain the true stored conversations for that period — answer from THAT block only, and never present this recent-context window's messages as belonging to that past period.
5. For anything older than this window, use the session_search tool when the user asks about past conversations.`;
  }
  return result;
}
