  ...
  },
{
  ...
  },
  {
    name: "volumeUp",
    description:
      "Increases the system volume by an EXACT percentage amount. Read the result to know the verified new level.",
    parameters: {
      type: Type.OBJECT,
      properties: {
        amount_percent: {
          type: Type.INTEGER,
          description:
            "Percentage points to increase (e.g. 5 for '+5%'). Default 10.",
        },
      },
      required: [],
    },
  },
  {
    name: "volumeDown",
    description:
      "Decreases the system volume by an EXACT percentage amount. Read the result to know the verified new level.",
    parameters: {
      type: Type.OBJECT,
      properties: {
        amount_percent: {
          type: Type.INTEGER,
          description:
            "Percentage points to decrease (e.g. 5 for '-5%'). Default 10.",
        },
      },
      required: [],
    },
  ...
  },
  {
    name: "getVolume",
    description:
      "Reads the CURRENT actual system master volume percentage (0-100) from the audio device. Use this to answer 'what is the volume right now' — never guess.",
    parameters: { type: Type.OBJECT, properties: {}, required: [] },
  },
  {
    name: "brightnessUp",
    description: "Increases screen brightness by a percentage amount.",
    parameters: {
      type: Type.OBJECT,
      properties: {
        amount: {
          type: Type.INTEGER,
          description:
            "Percentage points to increase (default 10). E.g. amount=5 raises brightness by exactly 5%.",
        },
      },
      required: [],
    },
  ...
  {
    name: "brightnessDown",
    description: "Decreases screen brightness by a percentage amount.",
    parameters: {
      type: Type.OBJECT,
      properties: {
        amount: {
          type: Type.INTEGER,
          description:
            "Percentage points to decrease (default 10). E.g. amount=5 lowers brightness by exactly 5%.",
        },
      },
      required: [],
    },
  },
  {
    name: "setBrightness",
    description:
      "Sets screen brightness to an exact percentage (0-100) and verifies the actual value afterwards.",
    parameters: {
      type: Type.OBJECT,
      properties: {
        percent: {
  ...
  {
    name: "getBrightness",
    description: "Reads the CURRENT actual screen brightness percentage (0-100).",
    parameters: { type: Type.OBJECT, properties: {}, required: [] },
  },
];

/**
 * Sabit task-lifecycle tools. These are appended ONLY to Sabit's Live session
 * tool list (see server.ts) — they were previously referenced in prompt text
 * and handled server-side but never DECLARED, so the model could never call
 * them and delegated tasks could never authoritatively complete.
 */
export const SABIT_LIFECYCLE_TOOLS = [
  {
    name: "sabitTaskComplete",
    description:
      "MUST be called as soon as the delegated task is FULLY executed and verified on screen. This is the ONLY way to authoritatively mark the task completed — the watchdog stops driving you the moment you call it.",
    parameters: {
      type: Type.OBJECT,
      properties: {
        summary: {
          type: Type.STRING,
          description:
            "A concise summary of the final result/outcome (what was done, key findings, or the answer to the user's request). This is delivered back to the user.",
        },
      },
      required: ["summary"],
    },
  },
  {
    ...
    description:
      "Call when the task cannot be completed due to a blocking issue (CAPTCHA, login wall, persistent error). Stops the task loop authoritatively.",
    parameters: {
      type: Type.OBJECT,
      properties: {
        reason: {
          type: Type.STRING,
          description: "The specific reason the task could not be completed.",
        },
      },
      required: ["reason"],
    },
  },
  {
    ...
    description:
      "Call when you need the user to do something on screen (log in, solve CAPTCHA, confirm a dialog) before you can continue. The user will be asked; you will receive their answer to continue from where you stopped.",
    parameters: {
      type: Type.OBJECT,
      properties: {
        message: {
          type: Type.STRING,
          description:
            "Exactly what the user needs to do on screen right now.",
        },
      },
      required: ["message"],
    },
...
/**
 * Maira's own task-completion tool. When Maira executes a compound sub-task
 * herself (Busy Rule), the continuation driver keeps firing on every
 * turnComplete until she calls this. Declared in the shared list but filtered
 * OUT of Sabit's tool list (Sabit uses sabitTaskComplete instead).
 */
export const MAIRA_TASK_TOOLS = [
  {
    name: "mairaTaskComplete",
    description:
      "MUST be called as soon as YOUR OWN active task (the one from the CONTINUE AUTOMATION TASK directive) is FULLY executed and verified. Ends the task loop. Do NOT call for ordinary chat replies.",
    parameters: {
      type: Type.OBJECT,
      properties: {
        summary: {
...
/**
 * Legacy `desktop*` tool names are declared to the model but are NOT the names
 * the Python agent registers. Previously such calls fell through to the client
 * stub, which replied {result:'ok'} without executing anything — the model then
 * truthfully-sounding-ly claimed actions that never happened. This map
 * canonicalizes them (name + argument renaming) before routing to the agent.
 */
export const DESKTOP_TOOL_ALIASES: ReadonlyMap<
  string,
  { tool: string; argMap?: Record<string, string> }
> = new Map([
  ['desktopOpenApp', { tool: 'openApplication', argMap: { app_name: 'name' } }],
  ['desktopClickOnText', { tool: 'clickOnText' }],
  ['desktopTypeText', { tool: 'typeText' }],
  ['desktopPressKey', { tool: 'pressKey' }],
  ['desktopScroll', { tool: 'scrollMouse' }],
  ['desktopScreenshot', { tool: 'takeScreenshot' }],
  ['desktopWebSearch', { tool: 'searchWeb' }],
]);

/** Resolve a model-called tool name/args to the canonical Python agent call. */
export function resolveDesktopTool(
  name: string,
  args: Record<string, unknown> | undefined,
): { name: string; args: Record<string, unknown> } {
  const alias = DESKTOP_TOOL_ALIASES.get(name);
  if (!alias) return { name, args: args || {} };
  const out: Record<string, unknown> = { ...(args || {}) };
  if (alias.argMap) {
    for (const [from, to] of Object.entries(alias.argMap)) {
  ...
  return { name: alias.tool, args: out };
}
