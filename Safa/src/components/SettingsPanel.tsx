...
export function SettingsPanel({ isOpen, onClose, settings, onChange, themeColor }: SettingsPanelProps) {
  ...
  const [soulConfig, setSoulConfig] = useState<{
    ...
    userName: "Nirob",
    preferredAddressing: "Nirob",
    personalPrompt: "",
  });
  const [soulSaving, setSoulSaving] = useState(false);
  const [soulSavedMsg, setSoulSavedMsg] = useState("");
    ...
    fetch("/api/soul")
      ...
      .then((data) => {
        if (data && typeof data === "object") {
          setSoulConfig({
            relationshipIdentity: data.relationshipIdentity || "Friend",
            userName: data.userName || "Nirob",
            preferredAddressing: data.preferredAddressing || "Nirob",
            personalPrompt: data.personalPrompt || "",
          });
        }
      })
      .catch(() => {});
  ...
  const saveSoul = async (updated: Partial<typeof soulConfig>) => {
    const newConfig = { ...soulConfig, ...updated };
    setSoulConfig(newConfig);
    setSoulSaving(true);
    setSoulSavedMsg("");
    try {
      ...
      setSoulSavedMsg("SOUL identity & relationship parameters saved!");
      setTimeout(() => setSoulSavedMsg(""), 3000);
    } catch (e) {
      /* ignore */
    } finally {
      setSoulSaving(false);
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              ...
              {activeTab === "general" && (
                <div className="space-y-4">
                  <div className="text-[10px] font-mono uppercase tracking-widest text-slate-500">
                    Startup &amp; User Identity
                  </div>

                  {/* Owner / User Name Input */}
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <input
                        type="text"
                        value={soulConfig.userName || "Nirob"}
                        onChange={(e) => saveSoul({ userName: e.target.value })}
                        placeholder="Nirob"
                    ...
                    <span className="text-[8px] text-slate-500 uppercase font-mono">
                      Set your display name (Owner / Creator)
                    </span>
                  </div>

                  {/* Protected Assistant Identity (Non-Editable) */}
                  <div className="p-3 rounded-xl border border-indigo-500/30 bg-indigo-500/10 space-y-2">
                    <div className="flex items-center gap-2 text-[10px] font-mono text-indigo-300 uppercase tracking-wider font-bold">
                      <Shield size={13} className="text-indigo-400" /> Protected Core Identity
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-[11px] font-mono">
                      <div className="p-2 rounded-lg bg-black/40 border border-indigo-500/20">
                        <span className="text-[9px] text-slate-400 block uppercase">Assistant Name</span>
                        <span className="text-white font-bold">Safa (সাফা)</span>
                      </div>
                      <div className="p-2 rounded-lg bg-black/40 border border-indigo-500/20">
                        <span className="text-[9px] text-slate-400 block uppercase">Creator &amp; Owner</span>
                        <span className="text-white font-bold">Nirob (নীরব)</span>
                      </div>
                    </div>
                    <span className="text-[8px] text-indigo-300/70 font-mono uppercase block">
                      This core identity is permanently protected and non-negotiable across all interactions.
                    </span>
                  </div>
                  ...
                  {settings.autoStart && (
                    <div className="mt-2 p-3 rounded-xl border border-emerald-500/20 bg-emerald-500/5 flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-mono text-emerald-300/80">
                        Safa will auto-launch on next login.
                      </span>
                    </div>
                  )}
                </div>
              ...
              {activeTab === "system" && (
                <div className="space-y-5">
                  ...
                  <ApiKeyField
                    label="Google Gemini API Key (Maira)"
                    description="Primary key — drives Maira's voice &amp; intelligence"
                    storageKey="gemini"
                    onSave={async (key) => {
                  ...
                  <BackupApiKeysSection />

                  <div className="h-px bg-white/5 my-4" />

                  <ApiKeyField
                    label="Google Gemini API Key (Sabit)"
                    description="Optional — separate key for Sabit's parallel sessions. Falls back to Maira's key if empty."
                    storageKey="sabit"
                    onSave={async (key) => {
                    ...
                    onClear={async () => {
                      const res = await fetch("/api/config/sabit/clear", {
                        method: "POST"
                      });
                  ...
                  <div className="p-3 rounded-xl border border-indigo-500/20 bg-indigo-500/5 flex items-start gap-2.5">
                    ...
                    <div>
                      <h4 className="text-[10px] font-bold font-mono text-indigo-300 uppercase">Dynamic Handoff</h4>
                      <p className="text-[9px] text-indigo-400 uppercase font-mono mt-0.5 leading-relaxed">
                        Maira and Sabit operate entirely independently and can execute parallel browser sessions safely without blocking one another.
                      </p>
                    </div>
                  </div>
                </div>
...
function ApiKeyField({
  ...
  useEffect(() => {
    // Check if key is configured currently on mount
    const checkStatus = async () => {
      try {
        const res = await fetch(storageKey === "sabit" ? "/api/config/sabit" : "/api/config");
        const data = await res.json();
        setHasStoredKey(storageKey === "sabit" ? data.hasCustomApiKey : data.hasApiKey);
      } catch {}
    };
    checkStatus();
  }, [storageKey]);
    ...
    <div className="space-y-1.5">
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className="px-3 py-2 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 text-xs font-bold uppercase tracking-wider transition disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer shrink-0"
        >
          {status === "saving" ? (
        ...
        {onClear && hasStoredKey && (
          <button
            onClick={handleClear}
            disabled={status === "saving"}
            className="px-3 py-2 rounded-xl border border-rose-500/30 hover:border-rose-500/60 bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 text-xs font-bold uppercase tracking-wider transition disabled:opacity-40 cursor-pointer shrink-0"
          >
            Clear
          </button>
        )}
      ...
      {status === "saved" && (
        <span className="block text-[9px] font-mono text-emerald-400">✓ Action completed successfully</span>
      )}
      {status === "error" && (
        <span className="block text-[9px] font-mono text-rose-400">✗ Operation failed</span>
      )}
    </div>
  );
}
...
/**
 * Backup API Keys — Visual-section-only failover pool management.
 * Lives directly below the Main (Maira) key. Saved keys stay visible as
 * numbered, always-editable masked input fields (same show/hide pattern as
 * ApiKeyField); the "Backup API Key" button sits below the list so more keys
 * can be added at any time.
 */
function BackupApiKeysSection() {
  interface BackupKey {
    id: string;
    key: string;
    createdAt: number;
  ...
  const [shows, setShows] = useState<Record<string, boolean>>({});
  ...
  const refresh = async () => {
    try {
      const res = await fetch("/api/config/backup-keys");
      const data = await res.json();
      const list: BackupKey[] = Array.isArray(data.keys) ? data.keys : [];
      setKeys(list);
      setEdits(prev => {
        const next: Record<string, string> = {};
        for (const k of list) next[k.id] = prev[k.id] ?? k.key;
        return next;
      });
    } catch {
      /* settings panel stays usable offline — list just stays as-is */
    } finally {
      setLoaded(true);
    }
  };
  ...
  const flash = (map: Record<string, "saving" | "saved" | "error">, id: string, status: "saved" | "error") => {
    map[id] = status;
    setRowStatus({ ...map });
    setTimeout(() => {
      delete map[id];
  ...
  const handleUpdate = async (id: string) => {
    const key = (edits[id] || "").trim();
    if (!key) return;
    const map = { ...rowStatus };
    map[id] = "saving";
    setRowStatus(map);
    try {
      const res = await fetch(`/api/config/backup-keys/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ key }),
      });
  ...
  const handleDelete = async (id: string) => {
    ...
    setRowStatus(map);
    try {
      const res = await fetch(`/api/config/backup-keys/${id}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
      await refresh();
    } catch {
      flash(map, id, "error");
  ...
  const keyInputClass =
    "w-full pl-9 pr-9 py-2 rounded-xl border border-white/10 bg-white/5 text-sm text-white font-mono focus:outline-none focus:border-cyan-400/50 transition";
  ...
  return (
    <div className="space-y-2.5">
      <div className="pt-1">
        ...
        <span className="text-[8px] text-slate-500 uppercase font-mono">
          Visual section only — failover pool. With backups added, visuals use Backup 1 → 2 → … → Main key.
        </span>
      </div>

      {loaded && keys.length === 0 && !adding && (
        <span className="block text-[9px] font-mono text-slate-600">
          No backup key added — visuals use the Main + Sabit keys (existing behavior).
        </span>
      )}
        ...
        <div key={k.id} className="space-y-1">
          <div className="flex gap-2">
            <div className="relative flex-1">
              <KeyRound className="absolute left-3 top-2.5 text-slate-400" size={14} />
              <input
                type={shows[k.id] ? "text" : "password"}
                value={edits[k.id] ?? k.key}
                onChange={(e) => setEdits(prev => ({ ...prev, [k.id]: e.target.value }))}
                spellCheck={false}
                className={keyInputClass}
              />
              <button
                onClick={() => setShows(prev => ({ ...prev, [k.id]: !prev[k.id] }))}
                className="absolute right-3 top-2.5 text-slate-400 hover:text-white transition"
                title={shows[k.id] ? "Hide key" : "Show key"}
              >
                {shows[k.id] ? <EyeOff size={14} /> : <Eye size={14} />}
              </button>
            </div>
            <button
              onClick={() => handleUpdate(k.id)}
              disabled={!(edits[k.id] ?? k.key).trim() || rowStatus[k.id] === "saving"}
              className="px-3 py-2 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 text-xs font-bold uppercase tracking-wider transition disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer shrink-0"
            >
              {rowStatus[k.id] === "saving" ? (
                <Loader2 size={14} className="animate-spin" />
              ) : rowStatus[k.id] === "saved" ? (
                <Check size={14} />
              ) : (
                "Save"
              )}
            ...
            <button
              ...
              disabled={rowStatus[k.id] === "saving"}
              className="px-3 py-2 rounded-xl border border-rose-500/30 hover:border-rose-500/60 bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 text-xs font-bold uppercase tracking-wider transition disabled:opacity-40 cursor-pointer shrink-0 flex items-center"
              title="Delete this backup key"
            >
              <Trash2 size={14} />
            </button>
          </div>
          <div className="flex items-center justify-between">
            ...
            {rowStatus[k.id] === "saved" && (
              <span className="text-[9px] font-mono text-emerald-400">✓ Saved</span>
            )}
            {rowStatus[k.id] === "error" && (
              <span className="text-[9px] font-mono text-rose-400">✗ Operation failed</span>
            )}
          </div>
        </div>
      ))}
      ...
      {adding && (
        <div className="space-y-1">
          <div className="flex gap-2">
            ...
            <button
              onClick={handleAdd}
              disabled={!newValue.trim() || addStatus === "saving"}
              className="px-3 py-2 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 text-xs font-bold uppercase tracking-wider transition disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer shrink-0"
            >
              {addStatus === "saving" ? (
                <Loader2 size={14} className="animate-spin" />
            ...
            <button
              onClick={() => { setAdding(false); setNewValue(""); setAddStatus("idle"); }}
              className="px-3 py-2 rounded-xl border border-white/10 hover:border-white/25 bg-white/5 hover:bg-white/10 text-slate-300 text-xs font-bold uppercase tracking-wider transition cursor-pointer shrink-0"
            >
              Cancel
            </button>
          </div>
          {addStatus === "error" && (
            <span className="block text-[9px] font-mono text-rose-400">✗ That key was rejected — check it and try again.</span>
          )}
        </div>
      )}
      ...
      <button
        ...
        disabled={adding}
        className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-cyan-400/30 hover:border-cyan-400/60 bg-cyan-500/5 hover:bg-cyan-500/10 text-cyan-300 text-[10px] font-bold font-mono uppercase tracking-wider transition disabled:opacity-40 cursor-pointer"
      >
        <Plus size={13} />
        Backup API Key
      </button>
