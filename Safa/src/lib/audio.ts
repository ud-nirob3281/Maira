...
import type { VisualItem } from './visualHubTypes';
...
export class MyraaAudioSession {
  ...
  private onTranscription: (role: "user" | "model", text: string, messageType?: string, isFinal?: boolean) => void;
  ...
  public onEmotionChange?: (emotion: string, intensity?: string) => void;
  public onSabitDelegated?: (event: any) => void;
  public onSabitTaskStatus?: (event: any) => void;
  ...
  // Fired when the server switches the active session (Stonic /resume & /branch).
  public onSessionSwitch?: (sessionId: string, messages: any[]) => void;

  // Fired when a Visual Hub item is created or changes state (generating →
  // ready/failed). Generation runs server-side, so these can arrive even while
  // the Visual Hub panel is closed.
  public onVisualUpdate?: (visual: VisualItem) => void;
  ...
  public mute() {
    ...
    if (this.outputGainNode && this.outputAudioCtx) {
      try {
        this.outputGainNode.gain.setValueAtTime(0, this.outputAudioCtx.currentTime);
      } catch (e) {}
    }
  }
  ...
  public unmute() {
    ...
    if (this.outputGainNode && this.outputAudioCtx) {
      try {
        this.outputGainNode.gain.setValueAtTime(1, this.outputAudioCtx.currentTime);
      } catch (e) {}
    }
  }
  ...
  constructor(handlers: {
    ...
    onTranscription: (role: "user" | "model", text: string, messageType?: string, isFinal?: boolean) => void;
    ...
    if (handlers.defaultMuted) {
      this.isMuted = true;
    }
    if (handlers.endpoint) {
      this.endpoint = handlers.endpoint;
    }
    try {
      const storageKey = this.endpoint === "/sabit-live" ? "sabit_session_id" : "maira_session_id";
      let existingSid = localStorage.getItem(storageKey);
      if (!existingSid) {
        existingSid = `session_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`;
        localStorage.setItem(storageKey, existingSid);
      }
      this.sessionId = existingSid;
    } catch (e) {
  ...
  public async connect(options?: {
    ...
    // Always re-read the UI's current session from localStorage. App.tsx
    // updates this key on "New Chat" and sidebar session switches BEFORE
    // calling disconnect()/connect(); reusing this.sessionId here kept the
    // voice WS attached to the OLD session so the restored history never
    // matched the chat shown in the UI.
    try {
      const storageKey = this.endpoint === "/sabit-live" ? "sabit_session_id" : "maira_session_id";
      const latestSid = localStorage.getItem(storageKey);
      if (latestSid) this.sessionId = latestSid;
    } catch {}
    ...
    try {
      ...
      this.ws = new WebSocket(`${protocol}//${window.location.host}${this.endpoint}${queryStr}`);
      ...
      this.ws.onopen = async () => {
        ...
        try {
          ...
          try {
            this.outputGainNode.gain.setValueAtTime(this.isMuted ? 0 : 1, this.outputAudioCtx.currentTime);
          } catch (e) {}
          ...
          this.micProcessorNode.onaudioprocess = (e) => {
            ...
            // Echo guard: while Safa is speaking, desktop-speaker audio can
            // leak back into the mic; Gemini's VAD then treats it as user
            // speech and interrupts the reply mid-way (long stories stopped
            // after a couple of lines even though the full transcript kept
            // streaming). While speaking, only forward mic audio when it is
            // loud enough to be a real person deliberately talking over
            // (barge-in), not attenuated echo.
            if (this.currentState === "speaking") {
              ...
              if (rms < 0.04) return; // too quiet to be deliberate speech — skip
            }
      ...
      this.ws.onmessage = async (event) => {
        try {
          ...
          // Handle server-side session switch (/resume, /branch)
          if (data.type === "session_switch" && data.sessionId) {
            try {
              const storageKey = this.endpoint === "/sabit-live" ? "sabit_session_id" : "maira_session_id";
            ...
            if (this.onSessionSwitch) {
              try { this.onSessionSwitch(data.sessionId, data.messages || []); } catch {}
            }
            return;
          }
          ...
          if (data.type === "transcription") {
            ...
            this.onTranscription(data.role, data.text, data.messageType, data.final === true);
          }
          ...
          // Handle Sabit task delegation
          if (data.type === "sabit_delegated") {
            if (this.onSabitDelegated) {
              this.onSabitDelegated(data);
            }
          }

          // Handle Sabit authoritative task status changes
          if (data.type === "sabit_task_status") {
            if (this.onSabitTaskStatus) {
              this.onSabitTaskStatus(data);
            }
          }

          // Handle emotion detection (model tool call or classifier fallback)
          ...
          // Handle Visual Hub pushes (new visual / state change)
          if (data.type === "visual_hub_update" && data.visual) {
            if (this.onVisualUpdate) {
              this.onVisualUpdate(data.visual as VisualItem);
            }
          }
  ...
  private reconnect() {
    ...
    // Jittered exponential backoff (Stonic pattern): base + random jitter
    // to prevent thundering herd on simultaneous reconnects.
    const base = Math.min(1500 * Math.pow(2, this.reconnectAttempts - 1), 30000);
  ...
  public disconnect() {
    this.isActivated = false;
    this.setState("disconnected");
    // Keep sessionId so reconnecting resumes the SAME session (Stonic pattern).
    // Clearing it here caused the server to mint a random sessionId on the next
    // connect — appearing as a brand-new conversation.
    if (this.sessionId) {
      try {
        const storageKey = this.endpoint === "/sabit-live" ? "sabit_session_id" : "maira_session_id";
