...
import {
  ...
  mergeVisual,
  type VisualItem,
} from './lib/visualHubTypes';

// Lazy-loaded so the Visual Hub (with mermaid/katex/recharts) costs nothing at
// startup — the bundle chunk is fetched only when the panel is first opened.
const VisualHubPanel = React.lazy(
  () => import('./components/visualhub/VisualHubPanel'),
);
...
function App() {
  ...
  const [showChat, setShowChat] = useState(true); // toggleable chat — default open
  const [showSoul, setShowSoul] = useState(false);

  // ── Visual Hub ──────────────────────────────────────────────────────────
  // The list lives here (not inside the panel) so visuals keep arriving while
  // the panel is closed — generation runs server-side and the panel just views
  // whatever state is current when reopened.
  const [showVisual, setShowVisual] = useState(false);
  const [visuals, setVisuals] = useState<VisualItem[]>([]);

  useEffect(() => {
    fetchVisuals().then(setVisuals);
  }, []);
  ...
  // Internal only — modulates how strongly the character video variant is
  // picked (slight/normal/strong). Never surfaced to the user.
  const [emotionIntensity, setEmotionIntensity] = useState<string | undefined>(undefined);
  ...
  // Id of the USER bubble currently receiving live speech-transcription chunks
  // (mirrors streamingBubbleIdRef for the model). A model chunk or a final
  // utterance closes it, so the next utterance starts a fresh bubble.
  const streamingUserBubbleIdRef = useRef<string | null>(null);
  ...
  const connect = useCallback(() => {
    ...
    const session = new MyraaAudioSession({
      ...
      onTranscription: (role, text, msgType, isFinal) => {
        const resolvedType = msgType || (role === 'user' ? 'user_voice' : 'safa_voice');
        const nowIso = new Date().toISOString();

        // ── Chat sidebar (live voice transcription + text, merged) ──
        // USER: chunks accumulate into ONE bubble per utterance; isFinal
        // replaces the partials with the canonical utterance text.
        // MODEL: accumulates into one streaming entry per turn.
        // The server still persists everything to SQLite / memory when the
        // Chat UI is hidden (panel unmounted → no UI processing cost).
        setChatMessages(prev => {
          if (role === 'user') {
            streamingBubbleIdRef.current = null; // next model turn = new bubble
            const uid = streamingUserBubbleIdRef.current;
            if (isFinal && uid) {
              const target = prev.find(m => m.id === uid);
              if (target) {
                return prev.map(m => (m.id === uid ? { ...m, text } : m));
              }
            }
            if (uid) {
              const lastUserMsg = prev.find(m => m.id === uid);
              if (lastUserMsg && lastUserMsg.messageType === resolvedType) {
                return prev.map(m =>
                  m.id === uid ? { ...m, text: m.text + text } : m
                );
              }
            }
            const newId = Math.random().toString(36).slice(2);
            streamingUserBubbleIdRef.current = newId;
            return [
              ...prev,
              {
                id: newId,
            ...
            streamingUserBubbleIdRef.current = null; // utterance boundary
      ...
      onEmotionChange: (emo, intensity) => {
        setEmotion(emo as MyraaEmotion);
    ...
    // Visual Hub pushes: a new visual (or a state change like generating →
    // ready) arrives from the server. Stonic-style auto-open on new items so
    // the user immediately sees what Safa created.
    session.onVisualUpdate = visual => {
      setVisuals(prev => mergeVisual(prev, visual));
      setShowVisual(true);
    };
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-black text-white font-sans">
      ...
      <div className="absolute top-4 right-4 z-30 flex items-center gap-2">
        ...
        {/* Visual Hub toggle — sits right of Connect/Disconnect */}
        <button
          onClick={() => setShowVisual(v => !v)}
          className={`relative flex items-center gap-1.5 px-3 py-2 rounded-xl border text-xs font-mono tracking-wider transition cursor-pointer ${
            showVisual
              ? 'border-cyan-400/40 bg-cyan-500/10 text-cyan-300'
              : 'border-white/10 bg-white/5 hover:bg-white/10 text-slate-300 hover:text-white'
          }`}
          title="Visual Hub — diagrams, images, math & charts by Safa"
        >
          <Eye size={14} />
          <span className="hidden sm:inline">VISUAL</span>
          {visuals.length > 0 && (
            <span className="ml-0.5 px-1.5 py-px rounded-md bg-cyan-500/15 border border-cyan-400/20 text-[9px] text-cyan-300 leading-none">
              {visuals.length}
            </span>
          )}
        </button>
      ...
      {/* Visual Hub — lazy chunk, rendered inside the app window */}
